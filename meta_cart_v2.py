"""
Meta-CART for Subgroup Discovery - Version 2.0 (FIXED)
=======================================================
...
- Propensity score adjustment for observational data
- Binary and survival outcome support
...
class MetaCART:
    ...
    min_samples_leaf : int, default=50
        Minimum number of samples required in each leaf node (increased from 30)
    min_samples_treatment_leaf : int, default=25
        Minimum number of treated samples in each leaf (increased from 10)
    min_samples_control_leaf : int, default=25
        Minimum number of control samples in each leaf (increased from 10)
    ...
    def _compute_treatment_effect(
        ...
        # Propensity weighting if available
        if propensity is not None:
            ...
            effect = np.average(y_treated, weights=weights_treated) - \
                     np.average(y_control, weights=weights_control)

            # Weighted variance
            var_treated = np.average(
                (y_treated - np.average(y_treated, weights=weights_treated))**2,
                weights=weights_treated
            )
            var_control = np.average(
                (y_control - np.average(y_control, weights=weights_control))**2,
        ...
        # P-value using Welch's t-test (unequal variances)
        if se > 0 and var_treated > 0 and var_control > 0:
            t_stat = effect / se
    ...
    def _subtree_error(self, node: Node) -> float:
        ...
        if node is None or node.is_leaf:
            return node.treatment_effect_se ** 2 * node.n_samples if node else 0.0
    ...
    def _prune_with_alpha(self, tree: Node, alpha: float) -> Node:
        ...
        node_error = tree.treatment_effect_se ** 2 * tree.n_samples
    ...
    def _evaluate_tree(
        ...
        for leaf_id in np.unique(leaf_assignments):
            ...
            # Squared error (lower is better, so we negate for maximization)
