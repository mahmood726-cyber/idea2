"""
Meta-CART for Subgroup Discovery - Version 3.0 (ALL BUGS FIXED)
================================================================
...
ALL CRITICAL BUGS FROM V2 FIXED:
✅ Bug #1: Alpha calculation now uses SE² (not SE² × n)
✅ Bug #2: Holm procedure enforces monotonicity
✅ Bug #3: IPW weights use Horvitz-Thompson (no estimand change)
✅ Bug #4: Evaluation metric uses SE² (pure precision)
✅ Bug #5: Survival outcomes raise NotImplementedError

NEW IN V3:
✅ Complete BootstrapStability class (see bootstrap.py)
✅ Propensity score diagnostics (balance + overlap)
✅ Binary outcomes properly implemented
...
class MetaCART:
    ...
    def _validate_inputs(
        ...
        # V3 FIX #5: Check outcome type and reject survival
    ...
    def _check_propensity_diagnostics(
        ...
        """
        V3 ENHANCEMENT: Check propensity score balance and overlap.
        ...
        # 2. Check covariate balance (Standardized Mean Differences)
        weights_treated = 1.0 / propensity[treatment == 1]
        weights_control = 1.0 / (1.0 - propensity[treatment == 0])

        # Normalize weights to sum to sample size (for SMD calculation)
        ...
        for j in range(X.shape[1]):
            ...
            # Weighted means
            mean_t_weighted = np.average(X_treated, weights=weights_treated)
            mean_c_weighted = np.average(X_control, weights=weights_control)
            ...
            if pooled_std > 0:
                smd[j] = abs(mean_t_weighted - mean_c_weighted) / pooled_std
            else:
                smd[j] = 0.0
        ...
        if imbalanced:
            warnings.warn(
                ...
                f"Consider including interactions or polynomial terms.",
                UserWarning
            )
    ...
    def _compute_treatment_effect(
        ...
        V3 ENHANCEMENT: Proper binary outcome handling with risk differences.
        ...
        # V3 ENHANCEMENT: Binary outcomes
        if self.outcome_type == 'binary':
            # Risk difference
            if propensity is not None:
                ...
                # V3 FIX #3: Horvitz-Thompson estimator (no normalization!)
                n_total = len(y)
                effect = (np.sum(y_treated / p_treated) / n_total -
                         np.sum(y_control / (1.0 - p_control)) / n_total)

                # Robust variance for IPW estimator
                mean_treated = np.sum(y_treated / p_treated) / n_total
                mean_control = np.sum(y_control / (1.0 - p_control)) / n_total

                var_treated = np.sum(((y_treated - mean_treated) / p_treated) ** 2) / (n_total ** 2)
                var_control = np.sum(((y_control - mean_control) / (1.0 - p_control)) ** 2) / (n_total ** 2)
                ...
                var_treated = p_treated_val * (1 - p_treated_val) / n1 if n1 > 0 else 0
                var_control = p_control_val * (1 - p_control_val) / n0 if n0 > 0 else 0
            ...
            if propensity is not None:
                ...
                # V3 FIX #3: Horvitz-Thompson estimator (correct!)
                n_total = len(y)
                effect = (np.sum(y_treated / p_treated) / n_total -
                         np.sum(y_control / (1.0 - p_control)) / n_total)

                # Robust variance for Horvitz-Thompson
                mean_treated = np.sum(y_treated / p_treated) / n_total
                mean_control = np.sum(y_control / (1.0 - p_control)) / n_total
    ...
    def _cv_prune_tree(
        ...
        """
        Cost-complexity cross-validation pruning.

        Implements the CART pruning algorithm:
        1. Generate sequence of nested trees by varying alpha
        2. Use CV to select optimal alpha
        ...
        # Select best alpha (minimize CV error, which is negative score)
    ...
    def _generate_alpha_sequence(self, tree: Node) -> List[float]:
        """
        Generate sequence of alpha values for cost-complexity pruning.
        ...
        def get_node_alpha(node):
            ...
            # where R is error and |T_node| is number of leaves

            # V3 FIX #1: Use SE² not SE² × n
            node_error = node.treatment_effect_se ** 2  # Variance, already scaled properly
            subtree_error = self._subtree_error(node)
    ...
    def _subtree_error(self, node: Node) -> float:
        """Compute total error in subtree."""
        if node is None or node.is_leaf:
            # V3 FIX #1: Use SE² not SE² × n
            return node.treatment_effect_se ** 2 if node else 0.0

        left_error = self._subtree_error(node.left_child)
        right_error = self._subtree_error(node.right_child)

        return left_error + right_error

    def _prune_with_alpha(self, tree: Node, alpha: float) -> Node:
        """
        Prune tree based on complexity parameter alpha.
        ...
        # V3 FIX #1: Use SE² not SE² × n
        node_error = tree.treatment_effect_se ** 2
        subtree_error = self._subtree_error(tree)
        num_leaves = tree.num_leaves

        if num_leaves > 1:
            cost_complexity_gain = (node_error - subtree_error) - alpha * (num_leaves - 1)
    ...
    def _evaluate_tree(
        ...
        """
        Evaluate tree on validation data.

        Returns negative mean squared error of treatment effect estimates.
        """
        if tree is None:
            return -np.inf
        ...
        for leaf_id in np.unique(leaf_assignments):
            ...
            p_leaf = propensity_val[mask] if propensity_val is not None else None

            if np.sum(t_leaf == 1) == 0 or np.sum(t_leaf == 0) == 0:
                continue

            effect, se, _ = self._compute_treatment_effect(y_leaf, t_leaf, p_leaf)

            # V3 FIX #4: Use SE² for pure precision (not SE² × n)
            error = se ** 2
            total_error += error

        return -total_error  # Negative because we minimize error
    ...
    def _apply_multiple_testing_correction(self) -> None:
        ...
        V3 FIX #2: Holm procedure now enforces monotonicity!
